...
productsController.postProduct = async (req, res) => {
  try {
    const { name, description, price, stock, sku, supplier } = req.validatedBody; // obtenemos los datos
    const newProduct = new Product({ name, description, price, stock, sku, supplier }); // creamos una nueva instancia del producto con los datos recibidos
